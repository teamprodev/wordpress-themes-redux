...
class Merlin {
    ...
    */
    protected function plugins() {
        ...
        // if(!$purchase_code) {
        //     echo '<p>' . esc_html__('Please verify the purchase code first.', 'digilab') . '</p>';
        //     exit(0);
        // }
    ...
    */
    public function _ajax_activate_license() {
        ...
        $request  = wp_remote_get('https://api.themefora.com/activate/'.trim($license_key)."/".$theme_name."/".$domain);
        $response = wp_remote_retrieve_body($request);
        $output = json_decode($response);

        switch ($output->status) {
            case 200:
                // add_option('envato_purchase_code_'.$output->item_id, $license_key, '', 'yes');

                // $options = get_option('digilab');
                // $options['disconnection'] = '0';
                // update_option('digilab', $options);
